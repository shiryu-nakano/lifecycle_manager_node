...
namespace transition_recipe_test
{
    ...
    private:
        ...
        {
            current_semantic_state_.semantic_state.clear();    // SemanticStateをクリア
            ...
            for (const auto &name : node_names_)
            {
                ...
                if (it == getstate_clients_.end()) // Nodeが持っているクライアントに，nameのものが無い場合
                {
                    ...
                    // このノード分は応答なしとして扱う
                    current_semantic_state_.semantic_state[name] = SemanticState::State::UNKNOWN;
                    if (pending_semantic_updates_ > 0)
                    {
            ...
            const rclcpp::Client<GetState>::SharedPtr &client)
        {
            if (!client->wait_for_service(500ms)) // 500ms待ってもサービスがなければタイムアウト
            {
                ...
                current_semantic_state_.semantic_state[node_name] = SemanticState::State::UNKNOWN;
                if (pending_semantic_updates_ > 0)
                {
                    --pending_semantic_updates_;
            ...
            client->async_send_request(
                ...
                [this, node_name](GetStateFuture future)
                {
                    try
                    {
                        ...
                        // ② SemanticState に反映
                        current_semantic_state_.semantic_state[node_name] =
        ...
        void maybe_log_semantic_state()
        {
            ...
            for (const auto &kv : current_semantic_state_.semantic_state)
            {
