...
#include "local_planning_manager/area_strategies/demo_a_strategy.hpp"
#include "local_planning_manager/area_strategies/demo_b_strategy.hpp"
#include "local_planning_manager/area_strategies/demo_c_strategy.hpp"
#include <limits>
#include <iostream>
#include <cmath>
...
namespace local_planning_manager
{
    ...
    // コンストラクタ
    // TODO
    DemoPlanningManagerComponent::LocalPlanningManagerComponent()
        : current_area_index_(0), current_area_strategy_(nullptr)
    {
        ...
        // デフォルトの切り替え条件
        registerAreaSwitchCondition("A", 10.0, 0.0, 2.0);  // A to B
        registerAreaSwitchCondition("B", 20.0, 0.0, 2.0); // B to C
    ...
    std::optional<TransitionRecipe> LocalPlanningManagerComponent::decideTransition(const TransitionContext &context)
    {
        ...
        if (current_area_strategy_ != nullptr)
        {
            ...
            std::cerr << "Warning: Current area strategy is null for area: " << current_area_id_ << std::endl;
        }
