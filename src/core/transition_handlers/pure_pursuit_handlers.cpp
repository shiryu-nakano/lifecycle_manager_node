...
PurePursuitToStoppingHandler::PurePursuitToStoppingHandler(double threshold)
    : threshold_(threshold) {}

std::optional<TransitionRecipe> PurePursuitToStoppingHandler::handle(const TransitionContext& ctx)
{
...
PurePursuitToDwaHandler::PurePursuitToDwaHandler(double threshold)
    : threshold_(threshold) {}

std::optional<TransitionRecipe> PurePursuitToDwaHandler::handle(const TransitionContext& ctx)
{
    if (ctx.current_state == "PurePursuit" &&
        ctx.obstacle_detected &&
        ctx.obstacle_distance <= threshold_)
